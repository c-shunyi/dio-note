...
export const register = async (req, res, next) => {
  try {
    ...
    if (userExist) return res.json(fail('用户名已存在'));
    // 3.判断密码是否符合要求（密码长度不能小于6位，不能大于16位）
    if (password.length < 6 || password.length > 16) return res.json(fail('密码长度不能小于6位，不能大于16位'));
    // 4.生成邀请码 
...
export const login = async (req, res, next) => {
  try {
    ...
    // if (!match) return res.json(fail('密码错误'));
    if (password !== user.password) return res.json(fail('密码错误'));
    // 4.生成token
...
export const getProfile = async (req, res, next) => {
  try {
    // 1.判断是否传入userId
    if (!req.userId) return res.json(fail('token异常'));
    // 2.根据userId查询用户
